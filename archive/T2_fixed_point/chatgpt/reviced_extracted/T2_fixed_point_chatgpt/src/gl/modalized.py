...
def is_modalized_in(A: Formula, p: str) -> bool:
    """Return True iff every occurrence of atom ``p`` is in the scope of Box.

    The formula language has no binders, so this is an occurrence-position
    check.  Occurrences under one or more boxes are allowed; unboxed
