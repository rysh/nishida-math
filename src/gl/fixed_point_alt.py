...
def fixed_point_alt(A: Formula, p: str) -> Formula:
    """Path-based alternative implementation of the fixed-point construction.

    This implementation deliberately avoids the prover, just like
    ``gl.fixed_point``.  It is still based on the same k-decomposition theorem:
    instead of introducing fresh placeholder atoms, it records concrete paths to
    the outermost p-containing boxes and performs simultaneous replacement at
    those paths.  Therefore the uniqueness tests using this module are best read
    as a structural-difference sanity check, not as a fully independent
    Bernardi/de Jongh alternative proof procedure.

    A more independent rank/trace implementation would be preferable, but it is
    not included here because the bounded-search alternative would require
    calling ``prove_gl`` inside the engine, which is explicitly out of bounds.
    """
